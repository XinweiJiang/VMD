...
% s = [exp(1+0.1*cos(2*pi*0.15*t+pi/3)).*sin(2*pi*(100*t + 1.5*t.^2 + 2/pi*sin(0.9*pi*t)));
%      exp(1+0.2*cos(2*pi*0.2*t+pi/4)).*sin(2*pi*(160*t - 1.5*t.^2 + 2/pi*sin(0.9*pi*t)));
%      exp(1+0.2*cos(2*pi*0.1*t+pi/5)).*sin(2*pi*(190*t + 8/pi*sin(0.9*pi*t)));
%      exp(1+0.3*cos(2*pi*0.2*t+pi/6)).*sin(2*pi*(200*t + 8/pi*sin(0.9*pi*t)))];     % IMF
...
% plot(abs(U1))
% subplot(212)
% instfrq = fs/(2*pi)*diff(unwrap(angle(U1)));
% plot(instfrq)
figure
hold on
color = ['r','g','b','m'];
for k = 1:K
    Uk= hilbert(U(k,:));
    instfrq = fs/(2*pi)*diff(unwrap(angle(Uk)));
    plot(t(1:end-1),instfrq,'LineWidth',1.4, 'color',color(k))
end
box on
xlabel('Time / Sec','fontsize',18)
ylabel('Frequency / Hz','fontsize',18)
